...
class AnnotationDiff:  # {{{
    def __init__(self, first, second, result=None):  # {{{
        self.first = first
        self.second = second
        if result is not None:
            self.result = result
        else:
            self.result = annotation.TextAnnotations(text=first.get_document_text(), source=str(second))
    ...
    def diff_relations(self):
        first_relations = dict(
            ((self.mapping.get_second(
                ...
                relation.arg2),
                relation.type),
                relation) for relation in self.first.get_relations())
        second_relations = dict(
            ((relation.arg1,
              relation.arg2,
              relation.type),
             relation) for relation in self.second.get_relations())
        first_relations_set = set(first_relations)
        second_relations_set = set(second_relations)

        for relation_fp in second_relations_set - first_relations_set:
            source, target, relation_type = relation_fp
            relation = second_relations[relation_fp]
            self.add_added(relation.id, 'Added relation')
        for relation_fp in first_relations_set - second_relations_set:
            source, target, relation_type = relation_fp
            relation = first_relations[relation_fp]
            clone = copy.copy(relation)
            clone.id = self.result.get_new_id('R')
            clone.arg1 = source
            clone.arg2 = target
            self.result.add_annotation(clone)
            self.add_missing(clone.id, 'Missing relation')
    # }}}
